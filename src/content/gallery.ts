 ...
 *
 * Ships live in v1 with zero photos; every album page renders correctly when
 * `ITEMS` is empty. Adding a photo: run the ingest script (strips EXIF
 * including GPS, resizes, converts, generates a blur placeholder, appends a
 * stub object here), then write the caption.
 *
 * Two standing rules once photos arrive: a yes from anyone identifiable
 * before it goes up, and EXIF/GPS stripped on the way in.
 */
 ...
 * `ITEMS` and never counted by `albumCount`; a real GalleryItem landing in
 * `ITEMS` for an album stops this from rendering there — see
 * `albumDisplayItems`.
 *
 * Generated by `pnpm placeholders:gallery`. Six shapes at varied aspect
 * ratios, cycled across however many preview tiles an album shows.
 */
...
export const GALLERY_COPY = {
  ...
  /** Sits above the placeholder preview grid. */
